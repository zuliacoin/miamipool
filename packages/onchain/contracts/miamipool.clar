...
(define-private (payout-fee)
    (let
        ...
        (begin
            (try! (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.citycoin-token send-many (unwrap-panic (as-max-len? payoutFeeList u5)))))   
            (ok true)
        )
    )
)
